...
class MPC_Controller:
    ...
    def move_c(self,Theta,Delta,state, dt=0, Gama=0):
        ...
        delta_x = F_r*Parameters['R_r']
        T_rolling = Parameters['m_v']*Parameters['g']*math.cos(Gama)*F_r*((Parameters['Eta_c']*Parameters['Eta_d'])/(Parameters['Tau_c']*Parameters['Tau_d']))*Parameters['R_r']           # Equivalent Torque of Rolling Resistance Force
        T_slope   = Parameters['m_v']*Parameters['g']*math.sin(Gama)*Parameters['R_r']*((Parameters['Eta_c']*Parameters['Eta_d'])/(Parameters['Tau_c']*Parameters['Tau_d']));                                    # Equivalent Torque of Slope Weight Force
        T_load = T_aero + T_rolling + T_slope;
        if T_engine <= T_load:
            T_engine = T_load
        ...
        # Runge Kuta 4 can be implemented!!
        W_engine1 = ((T_engine-T_load)/Parameters['I_eq'])*dt + W_engine0
        W_wheel1 = (1/(Parameters['Tau_c']*Parameters['Tau_d']))*W_engine1

        u1 = Parameters['R_r']*W_wheel1
        u_dot = (u1 - u0)/dt
        W_wheel_dot = (W_wheel1-W_wheel0)/dt
        ...
        F_z_2 = (Parameters['m_v']*(u_dot - v0*r0)*Parameters['h1']                 +\
                ...
                2*Parameters['I_each_wheel']*(W_wheel_dot)*math.cos(Delta)          +\
                F_xa*Parameters['h2']                                               +\
                Parameters['m_v']*Parameters['g']*math.sin(Gama)*Parameters['h1']   +\
                Parameters['m_v']*Parameters['g']*math.cos(Gama)*(Parameters['l1'] + delta_x))/(2*Parameters['l'])

        F_z_1 = (Parameters['m_v'] * Parameters['g']*math.cos(Gama) - 2*F_z_2)/2
        F_x_1 = -(Parameters['I_each_wheel']*W_wheel_dot + F_z_1*delta_x)/Parameters['R_r']
        ...
        r1 = r_dot * dt + r0
        psi1 = r1*dt + psi0
        x_g1 = (u1*math.cos(psi1) - v1*math.sin(psi1))*dt + x_g0
        y_g1 = (u1*math.sin(psi1) + v1*math.cos(psi1))*dt + y_g0
        # return np.array([[x_g1, y_g1, psi1, u1, v1, r1]]).T
    ...
    def mpc_cost(self, u_k, my_car, x_des, y_des, psi_des):
        dt = np.array(my_car.dt).copy()
        state = np.array([[my_car.x, my_car.y, my_car.psi, my_car.u, my_car.v, my_car.r]]).T
        u_k = u_k.reshape(self.horiz,2).T
        z_k = np.zeros((6,self.horiz+1))
        cost = 0.0
        desired_state = np.array([x_des,y_des])
        print(state)
        for i in range(self.horiz):
            state_dot = self.move_c(u_k[0,i],u_k[1,i],state)
            state = state + dt*state_dot
            z_k[:,i] = (state).reshape(6)
    ...
    def optimize(self, my_car, x_des, y_des, psi_des):
        bnd = [(0.1, 10),(np.deg2rad(-60), np.deg2rad(60))]*self.horiz
        # bnd = [(0.1, 1),(np.deg2rad(-60), np.deg2rad(60))]*self.horiz
        result = minimize(self.mpc_cost, args=(my_car, x_des, y_des, psi_des), x0 = np.zeros((2*self.horiz)), method='L-BFGS-B', bounds = bnd)
        print(result.x)
