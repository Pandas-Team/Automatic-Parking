...
class MPC_Controller:
    ...
    def constraint0(self, u_k):
        ...
        max_in = np.array([[10],[np.deg2rad(60)]])
        return np.sum((max_in**2-u_k[:,:1]**2))

    def mpc_cost(self, u_k, my_car, x_des, y_des, psi_des):
        test_car = Car_Dynamics(my_car.x, my_car.y, my_car.psi, my_car.u, my_car.v, my_car.r, length=4, dt=my_car.dt, Gama=0)
    ...
    def optimize(self, my_car, x_des, y_des, psi_des):
        ...
        result = minimize(self.mpc_cost, args=(my_car, x_des, y_des, psi_des), x0 = np.zeros((2*self.horiz)), method='SLSQP', constraints=[con0])
        ...
        # else:
        #     delta = np.deg2rad(-max_delta)
        return np.clip(result.x[0], 0.5,10),  np.clip(result.x[1], np.deg2rad(-60),np.deg2rad(60))
